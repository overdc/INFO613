...
{ 
   ...
   public int getMovieYear()
   {
      ...
      try
      {
          ...
          for (int iI = 0; iI < children.getLength(); iI++)
          {
        	  ...
        	  {
        		  year = Integer.parseInt(getNodeValue(childNode));
				  if (year < 0)
				  {
				      year = -1;
				  }
      ...
      for (int iI = 0; iI < children.getLength(); iI++)
      {
    	  ...
    	  if (childNode.getNodeName().equals("Directed_By"))
    	  {
    		  ...
    		  for (int jJ = 0; jJ < directorNodeList.getLength(); jJ++)
    		  {
    			  ...
    			  {
				      String dirString = getNodeValue(directorNode);
					  /* make sure the element is not blank */
					  if (!dirString.equals(""))
					  {
      ...
      for (int iI = 0; iI < children.getLength(); iI++)
      {
    	  ...
    	  if (childNode.getNodeName().equals("Genres"))
    	  {
    		  ...
    		  for (int jJ = 0; jJ < genreNodeList.getLength(); jJ++)
    		  {
    			  ...
    			  {
				      String genreString = getNodeValue(genreNode);
					  /* make sure the element was not blank */
    		  ...
    		  for (int jJ = 0; jJ < actorNodeList.getLength(); jJ++)
    		  {
    			  ...
    			  if (actorNode.getNodeName().equals("Actor"))
    			  {
    				  ...
    				  for (int kK = 0; kK < nameNodeList.getLength(); kK++)
    				  {
    					  ...
    					  {
    						  firstName = getNodeValue(nameNode);
    					  }
    					  else if (nameNode.getNodeName().equals("LastName"))
    					  {
    						  lastName = getNodeValue(nameNode);
    					  }    					  
    				  }
   ...
   * This function will get the String value for the element node passed in
   * @param node The element node to get the value from
   * @return String - the string value for the node or the empty string if it is null
   */
